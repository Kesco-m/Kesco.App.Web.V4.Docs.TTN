...
namespace Kesco.App.Web.Docs.TTN
{
    ...
    public partial class Nakladnaya : DocPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            _goPanel = PersonPanel.Init(this, SectionPrefixes.Go, GoInfo, GoCodeInfo, GO, GoAddress, DispatchPoint,
                GoStore, GoStoreInfo, GoNotes);
            ...
            _gpPanel = PersonPanel.Init(this, SectionPrefixes.Gp, GpInfo, GpCodeInfo, GP, GpAddress, DestinationPoint,
                GpStore, GpStoreInfo, GpNotes);
            GP.Changed += GP_Changed;

            //секция Поставщик
            _shipperPanel = PersonPanel.Init(this, SectionPrefixes.Shipper, ShipperInfo, ShipperCodeInfo, Shipper,
                ShipperAddress, null, ShipperStore, ShipperStoreInfo, null);
            Shipper.Changed += Shipper_Changed;

            //секция Плательщик
            _payerPanel = PersonPanel.Init(this, SectionPrefixes.Payer, PayerInfo, PayerCodeInfo, Payer, PayerAddress,
                null, PayerStore, PayerStoreInfo, null);
            ...
            //Секция Товары
            addResource.OnClick =
                string.Format("cmdasync('cmd','AddResource', 'PageId', '{0}', 'DocId', '{1}', 'MrisId', {2})", IDPage,
                    Document.Id, 0);
            rbProductSelect.Items.Add(new Item("0", " " + Resx.GetString("TTN_optImplementationByResources")));
            rbProductSelect.Items.Add(new Item("1", " " + Resx.GetString("TTN_optRealizationResourcesByWagons")));
            rbProductSelect.Items.Add(new Item("2", " " + Resx.GetString("TTN_optGroupByResource")));
            ...
            //Секция Услуги
            addFactUsl.OnClick =
                string.Format("cmd('cmd','AddFactUsl', 'PageId', '{0}', 'DocId', '{1}', 'FactUslId', {2})", IDPage,
                    Document.Id, 0);
            ...
            {
                JS.Write(@"accordionActive(0, 'false');");
            }
        ...
        public string RenderCorrectingDoc()
        {
            using (var w = new StringWriter())
            {
                ...
                {
                    //w.Write("<div class='predicate_block'>");
                    w.Write("<div class='marginL'>");
                    w.Write("<div id = 'CorrectingSequel'>");
                    ...
                    else
                    {
                        ...
                        w.Write(IsRusLocal
                            ? Document.CorrectingSequelDoc.TypeDocRu
                            : Document.CorrectingSequelDoc.TypeDocEn);

                        if (Document.CorrectingSequelDoc.Number.Length > 0)
                            w.Write(" № " + Document.CorrectingSequelDoc.Number);
                        if (Document.CorrectingSequelDoc.Date != DateTime.MinValue)
                            w.Write(" от " + Document.CorrectingSequelDoc.Date.ToString("dd.MM.yyyy"));
        ...
        public override bool DocEditable => base.DocEditable && !Document.IsCorrected;

        public Lib.Entities.Documents.EF.Trade.TTN Document => Doc as Lib.Entities.Documents.EF.Trade.TTN;
        ...
        public Dictionary<string, GetTitleDelegate> AccordionTitles = new Dictionary<string, GetTitleDelegate>();
        ...
        private readonly TextBox GoAddress = new TextBox();
        private readonly TextBox GpAddress = new TextBox();

        // итоговые значения табличных полей
        public decimal[,] ItogArray = new decimal[3, 3];

        public DBSPerson ShipperField => Shipper;

        public DBSPerson PayerField => Payer;
        ...
        public SectionTitlesClass SectionTitles
        {
            get
            {
                if (sectionTitles != null) return sectionTitles;
        ...
        public ControlNtfsClass ControlNtfs
        {
            get
            {
                if (controlNtfs != null) return controlNtfs;
        ...
        private List<V4PageCard> CardList
        {
            get
            {
                if (cardList != null) return cardList;
        ...
        protected override void EntityInitialization(Entity copy = null)
        {
            ...
            {
                Doc = (Lib.Entities.Documents.EF.Trade.TTN) copy;

                if (!CopyId.IsNullEmptyOrZero()) Entity.IsModified = true;
                ...
                RefreshMrisCurrentDoc(false, true, CopyId ?? CorrId);
                RefreshFactUslCurrentDoc(false, true, CopyId ?? CorrId);
        ...
        private void BindControls()
        {
            ...
            if (rbProductSelect.Value == string.Empty) rbProductSelect.Value = "0";
        ...
        private void SetStoreValueByFirstPosition()
        {
            ...
            {
                DBSShipperStore.Value = Document.PositionMris[0].ShipperStoreId > 0
                    ? Document.PositionMris[0].ShipperStoreId.ToString()
                    : "";
                DBSPayerStore.Value = Document.PositionMris[0].PayerStoreId > 0
                    ? Document.PositionMris[0].PayerStoreId.ToString()
                    : "";
            }
            else
            {
                DBSShipperStore.Value = "";
                DBSPayerStore.Value = "";
            }
        }
        ...
        private void SetInitValue()
        {
            ...
            else
            {
                var dogovor = "";
                var d = 0;
                foreach (var _bd in Document.BaseDocsLinks)
                {
        ...
        private string SetKursAndScale()
        {
            ...
            Document.KursField.Value = kurs == 1 ? "" : Convert.Decimal2Str(kurs, Document.CurrencyScale * 2);
        ...
        protected override void RefreshNotificationDocument()
        {
            ...
            _goPanel.RenderNtf();
            _gpPanel.RenderNtf();
            _payerPanel.RenderNtf();
            _shipperPanel.RenderNtf();
            var w = RenderKuratorSign(Contract.Value);
            JS.Write("if (gi('divKuratorSign')) gi('divKuratorSign').innerHTML={0};",
                HttpUtility.JavaScriptStringEncode(w, true));
        }

        protected override void PrepareDocToCopy(Document doc)
        {
            base.PrepareDocToCopy(doc);
            if (doc is Lib.Entities.Documents.EF.Trade.TTN)
            {
                ((Lib.Entities.Documents.EF.Trade.TTN) doc)._CorrectingDoc = string.Empty;
                ((Lib.Entities.Documents.EF.Trade.TTN) doc).CorrectingFlagField.Value = 0;
                ((Lib.Entities.Documents.EF.Trade.TTN) doc).CorrectingDocField.Value = "";
            }
        }
        ...
        protected override void SetDocMenuButtons()
        {
            ...
            var btnOldVersion = new Button
            {
                ...
                Width = 125,
                OnClick = string.Format("v4_windowOpen('{0}','_self');",
                    HttpUtility.JavaScriptStringEncode(
                        WebExtention.UriBuilder(ConfigurationManager.AppSettings["URI_Direction_OldVersion"],
                            CurrentQS)))
            };
        ...
        protected override void DocumentToControls()
        {
            ...
            if (CorrectableFlag.Checked && DocEditable)
            {
        ...
        protected override void SetControlProperties()
        {
            ...
            imgGoAddress.Visible =
                imgGpAddress.Visible = imgShipperAddress.Visible = imgPayerAddress.Visible = DocEditable;

            //SetHiddenForEmptyReadOnly
            if (!DocEditable || CorrectableFlag.Checked)
            {
                Hide("divAccountant");
                Hide("divAccountantDelete");
                Hide("divDirector");
                Hide("divDirectorDelete");
                Hide("divStoreKeeper");
                Hide("divStoreKeeperDelete");
            ...
            {
                Display("divAccountant");
                Display("divAccountantDelete");
                Display("divDirector");
                Display("divDirectorDelete");
                Display("divStoreKeeper");
                Display("divStoreKeeperDelete");

                Display("divGpToPayer");
                Display("divGoToShipper");
                Display("divShipperToGo");
                Display("divPayerToGp");
                if (efMonthOfResources.Value.IsNullEmptyOrZero())
            ...
            if (!DocEditable)
            {
                ...
                if (Invoice.SelectedItems.Count == 0) Hide("divInvoice");
                if (PaymentDocuments.SelectedItems.Count == 0) Hide("divPaymentDocuments");
        ...
        protected override void OnDocDateChanged(object sender, ProperyChangedEventArgs e)
        {
            ...
            if (Document.Date != DateTime.MinValue)
                Document.DateProvodkiField.Value = Document.Date;
        ...
        {
            RefreshMrisCurrentDoc(false, false);
            RefreshFactUslCurrentDoc(false, false);
        }

        /// <summary>
        ///     Обновляет табличное поле товаров
        /// </summary>
        public void RefreshMrisCurrentDoc(bool gotoLastPage, bool reloadPositions, string copyId = "")
        {
            FillMrisDataGrid(gotoLastPage);
            if (!string.IsNullOrEmpty(copyId) || reloadPositions) Document.LoadPositionMris(copyId);
            RenderItogTable();
        }
        ...
        /// </summary>
        public void RefreshFactUslCurrentDoc(bool gotoLastPage, bool reloadPositions, string copyId = "")
        {
            FillFactUslDataGrid(gotoLastPage);
            if (!string.IsNullOrEmpty(copyId) || reloadPositions) Document.LoadPositionFactUsl(copyId);
            RenderItogTable();
        }
        ...
        {
            var docId = System.Convert.ToInt32(copyId);
            Document.RefreshBaseDocs(docId);
            foreach (var item in Document.BaseDocsLinks)
            {
                item.Id = null;
                item.SequelDocId = 0;
            }
            ...
            PaymentDocuments.SelectedItems.AddRange(
                Document.GetDocLinksItems(Document.PlatezhkiField.DocFieldId, docId));
        }

        /// <summary>
        ///     Установка данных панели поставщик как у грузоотправителя или наоборот
        /// </summary>
        /// <param name="cmd">Команда</param>
        private void SyncShipperAndGo(string cmd)
        {
        ...
        /// <summary>
        ///     Установки данных панели плательщик как у грузополучателем
        /// </summary>
        /// <param name="cmd">Команда</param>
        private void SyncPayerAndGp(string cmd)
        {
        ...
        protected override void ProcessCommand(string cmd, NameValueCollection param)
        {
            ...
            switch (cmd)
            {
                ...
                case "AddResource":
                    ...
                    if (ValidateDocument(out validList))
                    {
                        ...
                        if (validList.Count > 0)
                            ...
                            AddResource(Resx.GetString("TTN_MrisTitle"), param["PageId"], param["DocId"],
                                param["MrisId"]);
                    }
                    else
                    {
                ...
                case "RefreshResource":
                    ...
                    {
                        RefreshMrisCurrentDoc(param["isNew"] == "True", param["isNew"] == "True");
                        if (param["closeForm"] == "True") JS.Write("resources_Records_Close({0});", param["ctrlFocus"]);
                        JS.Write("$('#addResource').focus();");
                    }
                ...
                case "RefreshResourceByVagon":
                    ...
                    {
                        RefreshMrisCurrentDoc(true, true);
                ...
                case "AddFactUsl":
                    ...
                    if (ValidateDocument(out validList, "AddFactUsl"))
                        AddService(Resx.GetString("TTN_FactUslTitle"), param["PageId"], param["DocId"],
                            param["FactUslId"]);
                    else
                        RenderErrors(validList, "<br/>" + Resx.GetString("TTN_msgEditingNoPossible"));
                    break;
                ...
                case "RefreshFactUsl":
                    ...
                    {
                        RefreshFactUslCurrentDoc(param["isNew"] == "True", param["isNew"] == "True");
                        //RefreshNotificationDocument();
                        if (param["closeForm"] == "True") JS.Write("services_Records_Close({0});", param["ctrlFocus"]);
                        JS.Write("$('#addFactUsl').focus();");
                    }
                    ...
                    if (ValidateDocument(out validList))
                    {
                        ...
                        else
                        {
                            ...
                            if ((Document.GOPersonField.Value ?? "").ToString().Length > 0)
                                sParams += "&goperson=" + Document.GOPersonField.Value;
                            if ((Document.GPPersonField.Value ?? "").ToString().Length > 0)
                                sParams += "&gpperson=" + Document.GPPersonField.Value;
                            if ((Document.GOPersonWeselField.Value ?? "").ToString().Length > 0)
                                sParams += "&gowesel=" + Document.GOPersonWeselField.Id;
                            if ((Document.GPPersonWeselField.Value ?? "").ToString().Length > 0)
                                sParams += "&gpwesel=" + Document.GPPersonWeselField.Id;

                            var parameters = string.Format("idpp={0}&idDoc={1}{2}", IDPage, Document.Id, sParams);
                            ReturnDialogResult.ShowAdvancedDialogSearch(this, "Select_Vagon", "GridResource",
                                Config.delivery_search, parameters, true, 0, 800, 600);
                        }
                    }
                    else
                    {
                ...
                case "GetContact":
                    ...
                    DBSContact.Focus();
                    JS.Write("personcontact_dialogShow('{0}','{1}','{2}','{3}','{4}');", param["Type"], position,
                        Resx.GetString("TTN_lblChoiceContact"), Resx.GetString("cmdApply"), Resx.GetString("cmdClose"));
                    break;
                case "SetAddress":
                    switch (param["Type"])
                    {
                ...
                case "GetPerson":
                    ...
                    JS.Write("person_dialogShow('{0}','{1}','{2}','{3}','{4}');", param["Type"], position,
                        Resx.GetString("TTN_lblChoiceEmployee"), Resx.GetString("cmdApply"),
                ...
                case "SetPerson":
                    ...
                    JS.Write("SetImgDeleteVisible('{0}','{1}','{2}');", Director.Value, Accountant.Value,
                        StoreKeeper.Value);
                    break;
                case "DeletePerson":
                    switch (param["Type"])
        ...
        /// </summary>
        private void FillMrisDataGrid(bool gotoLastPage)
        {
            ...
            switch (rbProductSelect.Value)
            {
                case "1":
                    ...
                    GridResource.SetDataSource(SQLQueries.SELECT_ID_DOC_ДвиженияНаСкладах_GRID, Config.DS_document,
                ...
                case "2":
                    ...
                    GridResource.SetDataSource(SQLQueries.SELECT_ID_DOC_ДвиженияНаСкладах_GRID_ПоРесурсу,
                        Config.DS_document, CommandType.Text, sqlParams);

                    GridResource.Settings.SetColumnDisplayVisible(
                        new List<string> {"КодРесурса", "КодЕдиницыИзмерения", "Точность", "КодСтавкиНДС"}, false);
                ...
                default:
                    ...
                    GridResource.Settings.SetColumnHref("СтранаПроисхождения", "КодСтраныПроисхождения",
                        Config.territory_form);
            ...
            //Заполняем контрол
            GridResource.RefreshGridData();
            if (gotoLastPage) GridResource.GoToLastPage();
            else GridResource.GoToPage(currentPage);
        ...
        private void FillFactUslDataGrid(bool gotoLastPage)
        {
            ...
            divNeedSaveDocumentPanelMris.Value =
                divNeedSaveDocumentPanelUsl.Value = Resx.GetString("TTN_msgNeedSaveDocument") + "!";
            ...
            GridUsl.SetDataSource(SQLQueries.SELECT_ID_DOC_ОказанныеУслуги_GRID, Config.DS_document, CommandType.Text,
                sqlParams);
            ...
            GridUsl.RefreshGridData();
            if (gotoLastPage) GridUsl.GoToLastPage();
            else GridUsl.GoToPage(currentPage);
        ...
        /// <summary>
        ///     Метод устанавливает значения полей Дата, Валюта, Поставщик и Плательщик в соответствии с полями объекта типа
        ///     Документ
        ///     Метод вызывается первым, затем следуют методы, которые конкретизируют тип документа и пытаются извлечь оттуда
        ///     больше данных
        /// </summary>
        ...
        private bool FillByDocument(Document d)
        {
            ...
            if (string.IsNullOrEmpty(Shipper.Value))
            {
                ...
                if (card != null)
                {
                    ...
                    ShipperAddress.Value = card.АдресЮридический.Length == 0
            ...
            if (string.IsNullOrEmpty(Payer.Value))
            {
                ...
                if (card != null)
                {
                    ...
                    PayerAddress.Value = card.АдресЮридический.Length == 0
                        ? card.АдресЮридическийЛат
                        : card.АдресЮридический;
        ...
        private bool FillByDogovor(Dogovor d)
        {
            ...
            if (string.IsNullOrEmpty(ShipperStore.Value))
            {
                ...
                if (Shipper.Value == d.Person1Field.Value.SafeToString() ||
                    Payer.Value == d.Person2Field.Value.SafeToString())
                    ShipperStore.BindDocField.Value = d.Sklad1Field.Value;
                else if (Shipper.Value == d.Person2Field.Value.SafeToString() ||
                         Payer.Value == d.Person1Field.Value.SafeToString())
                    ShipperStore.BindDocField.Value = d.Sklad2Field.Value;
                else if (string.IsNullOrEmpty(PayerStore.Value) ||
                         PayerStore.Value == d.Sklad2Field.Value.SafeToString())
                    ShipperStore.BindDocField.Value = d.Sklad1Field.Value;
                else
                    ShipperStore.BindDocField.Value = d.Sklad2Field.Value;
            ...
            if (string.IsNullOrEmpty(PayerStore.Value))
            {
                ...
                if (ShipperStore.Value == d.Sklad1Field.Value.SafeToString() ||
                    Shipper.Value == d.Person1Field.Value.SafeToString())
                {
        ...
        private void FillByCorrectableTtn(Lib.Entities.Documents.EF.Trade.TTN ttn)
        {
            CorrId = ttn.Id;
            var oldId = Doc.Id;
            var oldNumber = Doc.Number;
            var oldDate = Doc.Date;
            var cloned = (Lib.Entities.Documents.EF.Trade.TTN) ttn.Clone();
        ...
        /// <summary>
        ///     Метод устанавливает поля ГО и ГП из последней накладной с такими же поставщиком, плательщиком и договором, если она
        ///     существует
        /// </summary>
        private void TryFindGOGP()
        {
            ...
            var sqlParams = new Dictionary<string, object>
            {
                {"@КодТипаДокумента", DocTypeEnum.ТоварноТранспортнаяНакладная}, {"@КодЛица1", Shipper.Value},
                {"@КодЛица2", Payer.Value}, {"@КодДокументаОснования", Contract.Value}
            };

            var dt = DBManager.GetData(SQLQueries.SELECT_ПоследнийДокументПоТипу, Config.DS_document, CommandType.Text,
                sqlParams);
            if (null == dt) return;

            if (dt.Rows.Count > 0)
            {
                var odb = dt.Rows[0]["КодЛица3"];
                if (odb != DBNull.Value)
        ...
        /// </summary>
        private void SetAccortdionTitles()
        {
            ...
            foreach (var p in AccordionTitles) ClientScripts.SendSetInnerHtml(this, p.Key, p.Value());
        }
        ...
        public static string GetCtrlText(V4Control ctrl, bool fEncoded, Employee currentUser)
        {
            ...
            if (ctrl is Select)
            {
                ...
                if (sc.IsMultiSelect)
                {
                    foreach (var i in sc.SelectedItems)
                    {
                        ...
                        else if (o is Lib.Entities.Item)
                        {
                            AddTitle(((Lib.Entities.Item) o).Value.ToString(), fEncoded, ref sbValues);
                        }
                        else
                        {
                            var to = o.GetType();
                            var oText = to.GetProperty(sc.ValueField).GetValue(o, null);
                            if (null != oText) AddTitle(oText.ToString(), fEncoded, ref sbValues);
                        }
                    }
                }
                else
        ...
        public string GetSectionHtmlDescription(IEnumerable<V4Control> ctrls, bool newLine = false)
        {
            ...
            foreach (var ctrl in ctrls)
            {
                ...
                var textOfControl = GetCtrlText(ctrl, true, CurrentUser);
                ...
                {
                    if (sbDescription.Length > 0) sbDescription.Append(newLine ? "<br/>" : sep);
        ...
        private string CreateTitleMultiLineDescription(IEnumerable<V4Control>[] ctrls, bool newLine = false)
        {
            var sb = new StringBuilder();

            //Уведомление добавляется как <div> элемент, поэтому после него не требуется добавлять <br/> перед следующим элементом
            var fNtfAdded = false;
            foreach (var ctrl_array in ctrls)
            {
                var dataInfo = GetSectionHtmlDescription(ctrl_array, newLine);

                using (var ntfText = new StringWriter())
                {
                    var strNtfInfo = string.Empty;

                    foreach (var ctrl in ctrl_array) ctrl.RenderNtf(ntfText);
                    ...
                    var fData = dataInfo.Length > 0 || strNtfInfo.Length > 0;
        ...
        {
            V4Control[][] ctrls =
                {new V4Control[] {Driver, Car, CarNumber, TrailerNumber}, new V4Control[] {PowerOfAttorney, CarTtn}};
        ...
        {
            V4Control[] ctrls2 =
                {Director, DirectorPosition, Accountant, AccountantPosition, StoreKeeper, StoreKeeperPosition};

            var sbDescription2 = new StringBuilder();

            for (var i = 0; i < ctrls2.Length - 1; i += 2)
            {
            ...
            if (sbDescription2.Length > 0) innerHtml.Append(" " + SectionTitles.SignersOfPaper + ":");
        ...
        private string GeneralDataGetTitle()
        {
        ...
        protected string DocumentsGetTitle()
        {
            V4Control[][] ctrls =
            {
                new V4Control[] {Contract}, new V4Control[] {Enclosure}, new V4Control[] {ApplicationForPurchasing},
                new V4Control[] {LetterOfCredit}, new V4Control[] {BillOfLading}, new V4Control[] {Invoice},
        ...
        private void StoreShipper_BeforeSearch(object sender)
        {
            ...
            DBSShipperStore.Filter.IsWarehouseType.Enabled = true;
        ...
        private void StorePayer_BeforeSearch(object sender)
        {
            ...
            DBSPayerStore.Filter.IsWarehouseType.Enabled = true;
        ...
        {
            ApplicationForPurchasing.Filter.Type.DocTypeParams.AddRange(
                GetControlTypeFilter(Document.ZvkBField.DocFieldId));
        ...
        {
            BillOfLading.Filter.Type.DocTypeParams.AddRange(
                GetControlTypeFilter(Document.BillOfLadingField.DocFieldId));
        ...
        {
            PaymentDocuments.Filter.Type.DocTypeParams.AddRange(
                GetControlTypeFilter(Document.PlatezhkiField.DocFieldId));
        }
        ...
        private void LinkedDoc_BeforeSearch(DBSDocument ctrl, DBSDocument link)
        {
            ctrl.Filter.LinkedDoc.LinkedDocParams.Clear();

            if (link.IsMultiSelect)
            {
                foreach (var i in link.SelectedItems)
                {
                    var lp1 = new LinkedDocParam {QueryType = LinkedDocsType.DirectReasons, DocID = i.Id};
                    var lp2 = new LinkedDocParam {QueryType = LinkedDocsType.DirectСonsequences, DocID = i.Id};
            ...
            var lp3 = new LinkedDocParam {QueryType = LinkedDocsType.DirectReasons, DocID = link.Value};
            var lp4 = new LinkedDocParam {QueryType = LinkedDocsType.DirectСonsequences, DocID = link.Value};

            ctrl.Filter.LinkedDoc.LinkedDocParams.Add(lp3);
            ctrl.Filter.LinkedDoc.LinkedDocParams.Add(lp4);
        }

        private void CarTtn_BeforeSearch(DBSDocument ctrl)
        {
        ...
        {
            sender.Filter.PersonValidAt = Doc.Date == DateTime.MinValue ? DateTime.Today : Doc.Date;
        }
        ...
        {
            DBSPerson.Filter.PersonType = 2;
            DBSPerson.Filter.PersonValidAt = Document.Date == DateTime.MinValue ? DateTime.Today : Document.Date;
        ...
        public void Person_OnRenderNtf(Ntf ntf, DBSPerson personCtrl, string prefix)
        {
            ...
            if (p == null || p.Unavailable)
            {
                ...
                {
                    Message = Resx.GetString("_Msg_ЛицоНеДоступно"),
            ...
            if (!p.IsChecked)
                ntf.Add(new Notification
                {
                    Message = Resx.GetString("TTN_ntfPersonNotTested"),
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            ...
            if (crd != null)
            {
                if (crd.NameLat.Length == 0 && crd.NameRus.Length == 0)
                    ntf.Add(new Notification
                    {
                        Message = Resx.GetString("_Msg_ЛицоНазвание"),
                        Status = NtfStatus.Error,
                        DashSpace = true
                    });
                ...
                if (prefix == "Shipper" || prefix == "Payer")
                    if (crd.АдресЮридический.Length == 0 && crd.АдресЮридическийЛат.Length == 0)
                        ntf.Add(new Notification
                        {
                            Message = Resx.GetString("_Msg_ЛицоАдресЮридический"),
                            Status = NtfStatus.Error,
                            DashSpace = true
                        });
            }
            else
            {
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.PersonDate,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            }
            ...
            if (prefix == "Shipper" || prefix == "Payer")
            {
                ...
                if (contract != null && !contract.Unavailable)
                    if (contract.GetPersonIndex(personCtrl.Value) <= (contract.DataUnavailable ? -1 : 0) &&
                        (contract.DataUnavailable || contract.VidVzaimoraschetovField.Value.ToString() == "0"))
                        ntf.Add(new Notification
                        {
                            Message = Resx.GetString("TTN_nntfPersonNotMatchContract"),
                            Status = NtfStatus.Error,
                            DashSpace = true
                        });
                ...
                if (enclosure != null && !enclosure.Unavailable)
                    if (enclosure.GetPersonIndex(personCtrl.Value) <= (enclosure.DataUnavailable ? -1 : 0) &&
                        (enclosure.DataUnavailable || enclosure.VidVzaimoraschetovField.Value.ToString() == "1"))
                        ntf.Add(new Notification
                        {
                            Message = Resx.GetString("TTN_nntfPersonNotMatchEnclosure"),
                            Status = NtfStatus.Error,
                            DashSpace = true
                        });
            }
        ...
        public void Address_OnRenderNtf(Ntf ntf, DBSPerson personCtrl, DBSPersonContact addrCtrl, string prefix)
        {
            ...
            var sqlParams = new Dictionary<string, object> {{"@Дата", Doc.Date}, {"@КодКонтакта", addrCtrl.Value}};

            var retObj = DBManager.ExecuteScalar(SQLQueries.SELECT_TEST_КонтактДействует, CommandType.Text,
                Config.DS_person, sqlParams);
            if (retObj != DBNull.Value && (int) retObj == 0)
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.AddressDate,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            ...
            if (personCtrl.ValueInt.HasValue)
            {
                sqlParams = new Dictionary<string, object>
                    {{"@КодЛица", personCtrl.ValueInt}, {"@КодКонтакта", addrCtrl.Value}};

                retObj = DBManager.ExecuteScalar(SQLQueries.SELECT_TEST_КонтактЛица, CommandType.Text, Config.DS_person,
                    sqlParams);
                if (retObj != DBNull.Value && (int) retObj == 0)
                    ntf.Add(new Notification
                    {
                        Message = ControlNtfs.PersonAddress[prefix],
                        Status = NtfStatus.Error,
                        DashSpace = true
                    });
            }
        ...
        public void Store_OnRenderNtf(Ntf ntf, DBSPerson personCtrl, Store store, string prefix)
        {
            ...
            if (store.Unavailable)
            {
                ...
                ntf.Add(new Notification
                {
                    Message = Resx.GetString("TTN_msgAccountNotAvailable"),
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
                return;
            ...
            if (!string.IsNullOrEmpty(Currency.Value) && Currency.ValueInt != store.ResourceId)
                ntf.Add(new Notification
                {
                    Message = Resx.GetString("TTN_ntfCurrencyDocument"),
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            ...
            if (!store.IsAlive(documentDate))
                ntf.Add(new Notification
                {
                    Message = Resx.GetString("TTN_ntfStoreAccountDate"),
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            ...
            if (prefix == "Shipper")
            {
                ...
                if (contract != null && !contract.DataUnavailable)
                    if (personCtrl.Value == contract.Person1Field.Value.ToString() &&
                        ...
                        ntf.Add(new Notification
                        {
                            Message = Resx.GetString("TTN_ntfPersonStoreDocument"),
                            Status = NtfStatus.Error,
                            DashSpace = true
                        });
                ...
                if (enclosure != null && !enclosure.DataUnavailable)
                    if (personCtrl.Value == enclosure.Person1Field.Value.ToString() &&
                        ...
                        ntf.Add(new Notification
                        {
                            Message = Resx.GetString("TTN_ntfPersonStoreEnclosure"),
                            Status = NtfStatus.Error,
                            DashSpace = true
                        });
            }
            ...
            if (store.KeeperId > 0)
            {
                ...
                if (!p.IsChecked)
                    ntf.Add(new Notification
                    {
                        Message = Resx.GetString("_Msg_СчетБанкНеПроверен"),
                        Status = NtfStatus.Error,
                        DashSpace = true
                    });
                ...
                if (card == null)
                    ntf.Add(new Notification
                    {
                        Message = string.Format(Resx.GetString("_Msg_СчетРеквизитыБанка"),
            ...
            if (personCtrl.ValueInt.HasValue && store.ManagerId != personCtrl.ValueInt)
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.PersonStore[prefix],
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
        }
        ...
        protected void DateOfPosting_OnRenderNtf(object sender, Ntf ntf)
        {
            ...
            if (DateOfPosting.ValueDate < Doc.Date.Date)
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.PostingOnDate,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
        }
        ...
        public void Currency_OnRenderNtf(object sender, Ntf ntf)
        {
            ...
            if (!Currency.Value.IsNullEmptyOrZero() && contract != null && !contract.Valyuta.IsNullEmptyOrZero())
                if (Currency.Value != contract.Valyuta)
                    ntf.Add(new Notification
                    {
                        Message = ControlNtfs.CurrencyContract,
                        Status = NtfStatus.Error,
                        DashSpace = true
                    });
        }
        ...
        protected void PaymentDocuments_OnRenderNtf(object sender, Ntf ntf)
        {
            ...
            var sb_invoces = new StringBuilder();

            foreach (var i in Invoice.SelectedItems)
            {
                if (sb_invoces.Length > 0) sb_invoces.Append(",");
                sb_invoces.Append(i.Id);
            }

            var sb_payments = new StringBuilder();

            foreach (var i in PaymentDocuments.SelectedItems)
            {
            ...
            var sqlParams = new Dictionary<string, object>
                {{"@КодыОснований", sb_invoces.ToString()}, {"@Коды", sb_payments.ToString()}};

            var retObj = DBManager.ExecuteScalar(SQLQueries.SELECT_TEST_СвязиДокументовВытекающие, CommandType.Text,
                Config.DS_document, sqlParams);
            if (retObj != DBNull.Value && (int) retObj == 0)
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.PaymentInvoice,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
        }
        ...
        protected void Enclosure_OnRenderNtf(object sender, Ntf ntf)
        {
            ...
            foreach (var i in Enclosure.SelectedItems)
            {
            ...
            var sqlParams = new Dictionary<string, object>
                {{"@КодыОснований", Contract.Value}, {"@Коды", sb.ToString()}};
            ...
            if (retObj != DBNull.Value && (int) retObj == 0)
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.EnclosureContract,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
        }
        ...
        protected void Document_OnRenderNtf(object sender, Ntf ntf)
        {
            ...
            if (d.DataUnavailable)
            {
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.DocumentForm,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            }
            ...
            else
            {
                if (dbsdoc.ID == "Contract" && Document.DogovorField.ValueString.Length != 0)
                {
                    ...
                    if (contract != null && contract.KuratorField.ValueString.Length > 0)
                    {
                        var k = GetObjectById(typeof(Employee), contract.KuratorField.ValueString) as Employee;
            ...
            if (d.Date > Doc.Date)
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.BaseDocumentOnDate,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
            ...
            if (d.GetPersonIndex(Shipper.Value) <= (d.DataUnavailable ? -1 : 0) ||
                ...
                ntf.Add(new Notification
                {
                    Message = ControlNtfs.PersonDocument,
                    Status = NtfStatus.Error,
                    DashSpace = true
                });
        }
        ...
        /// <summary>
        ///     Метод обновляет уведомления у элементов управления не входящих в панели Грузоотправитель, Грузополучатель,
        ///     Поставщик, Плательщик
        /// </summary>
        private void RenderNonPanelControlsNtf()
        {
            V4Control[] ctrls =
            {
                //Данные накладной
        ...
        protected void CorrectableTtn_Changed(object sender, ProperyChangedEventArgs e)
        {
            if (e != null)
            {
                ...
                if (CorrectableTtn.ValueText != "")
                {
                    ...
                    var col = correctingNakl.GetSequelDocs(System.Convert.ToInt32(Document.CorrectingDocField.Id));
                    if (col.Count > 1)
                    {
                        ShowMessage(Resx.GetString("TTN_msg_Больше1Корректирующего"),
                            Resx.GetString("errDoisserWarrning"));
                        CorrectingDoc_InfoClear(e.OldValue);
                        return;
                    }
                    ...
                    if (col.Count == 1 && col[0].Id != Document.Id)
                    {
                        ...
                        do
                        {
                            ...
                            col = correctingNakl.GetSequelDocs(System.Convert.ToInt32(Document.CorrectingDocField.Id));
                        } while (col.Count >= 1 && col[0].Id != Document.Id);

                        addMessage =
                            string.Format(
                                Resx.GetString("MSG_DocAlreadyCorrected") + "! " +
                                Resx.GetString("MSG_LastDocAsCorrected"), original, CorrectableTtn.ValueText);
                    }
                    else
                    {
                    ...
                    ShowConfirm(
                        string.Format(addMessage + " " + Resx.GetString("MSG_DataMergedWithCorrected"),
                            CorrectableTtn.ValueText),
                        Resx.GetString("errDoisserWarrning"),
                        Resx.GetString("CONFIRM_StdCaptionYes"),
                        Resx.GetString("CONFIRM_StdCaptionNo"),
        ...
        protected void Shipper_Changed(object sender, ProperyChangedEventArgs e)
        {
            ...
            var dt = DBManager.GetData(SQLQueries.SELECT_ПоследнийИзмДокументПоТипу, Config.DS_document,
                CommandType.Text, sqlParams);
            if (null == dt) return;
        ...
        {
            DBSDocument[] docCtrls =
            {
                CarTtn, Contract, Enclosure, ApplicationForPurchasing, LetterOfCredit, BillOfLading, Invoice,
                PaymentDocuments
            };

            foreach (var docCtrl in docCtrls) docCtrl.RenderNtf();

            if (V4IsPostBack)
            {
                // удаляем договор при изменении поставщика или плательщика
                Contract.BindDocField.Value = "";
                Contract_Changed(null, null);
                Contract.RenderNtf();
            }
        ...
        protected void Contract_Changed(object sender, ProperyChangedEventArgs e)
        {
            ...
            else
            {
                ...
                if (Currency.Value.IsNullEmptyOrZero()) Currency.BindDocField.Value = d.Valyuta;
            }
            ...
            if (Contract.Value.Length > 0 && Enclosure.Value.Length == 0)
            {
                ...
                if (Enclosure.TryFindSingleValue()) Enclosure.BindDocField.Value = Enclosure.ValueInt;
        ...
        protected void ShipperStore_Changed(object sender, ProperyChangedEventArgs e)
        {
            if (Document.PositionMris != null && Document.PositionMris.Count > 0)
                if (DBSShipperStore.Value != string.Empty ||
                    DBSShipperStore.Value == string.Empty && !DBSShipperStore.IsRequired)
                    Document.PositionMris.ForEach(delegate(Mris p)
                    {
                        if (DBSShipperStore.Value == string.Empty && p.ShipperStoreId != null ||
                            p.ShipperStoreId != int.Parse(DBSShipperStore.Value))
                        {
                            p.ShipperStoreId = Convert.Str2IntNullable(DBSShipperStore.Value);
        ...
        protected void PayerStore_Changed(object sender, EventArgs e)
        {
            if (Document.PositionMris != null && Document.PositionMris.Count > 0 && DBSPayerStore.Value != string.Empty)
                if (DBSPayerStore.Value != string.Empty ||
                    DBSPayerStore.Value == string.Empty && !DBSPayerStore.IsRequired)
                    Document.PositionMris.ForEach(delegate(Mris p)
                    {
                        if (DBSPayerStore.Value == string.Empty && p.PayerStoreId != null ||
                            p.PayerStoreId != int.Parse(DBSPayerStore.Value))
                        {
                            p.PayerStoreId = Convert.Str2IntNullable(DBSPayerStore.Value);
                            if (!p.Id.IsNullEmptyOrZero()) p.Save(false);
                        }
                    });
        ...
        private void LinkedDoc_Changed1(DBSDocument ctrl, DBSDocument link)
        {
            if (link.SelectedItems.Count < 1)
            {
                var sb = new StringBuilder();

                foreach (var i in ctrl.SelectedItems)
                {
                    if (sb.Length > 0) sb.Append(",");
                    sb.Append(i.Id);
                }
                ...
                var dt = DBManager.GetData(SQLQueries.SELECT_СвязиДокументовОснованияДляВытекающих, Config.DS_document,
                    CommandType.Text, sqlParams);
                if (null != dt)
                {
                    link.SelectedItems.AddRange(dt.AsEnumerable().Select(row =>
                    {
                        var id = ((int) row["КодДокумента"]).ToString();
                        return new Lib.Entities.Item {Id = id, Value = link.GetObjectById(id)};
                    }));
        ...
        private void LinkedDoc_Changed2(DBSDocument ctrl, DBSDocument link)
        {
            if (link.SelectedItems.Count < 1)
            {
                ...
                var dt = DBManager.GetData(SQLQueries.SELECT_СвязиДокументовВытекающиеИзОснований, Config.DS_document,
                    CommandType.Text, sqlParams);
                if (null != dt)
                {
                    link.SelectedItems.AddRange(dt.AsEnumerable().Select(row =>
        ...
        protected void Invoice_ValueChanged(object sender, ProperyChangedEventArgs e)
        {
            if (!e.NewValue.IsNullEmptyOrZero())
            {
                ...
                Document.AddBaseDoc(value.ToString(), Document.SchetPredField.DocFieldId);

                var dtpList = GetControlTypeFilter(Document.PlatezhkiField.DocFieldId).Aggregate(string.Empty,
                    (current, dtp) => current + (current == string.Empty ? dtp.DocTypeID : "," + dtp.DocTypeID));
                var sql = SQLQueries.SELECT_СвязиДокументовВытекающиеИзОснованийПоТипу(e.NewValue,
                    Document.PlatezhkiField.DocFieldId.ToString(), dtpList);
                ...
                if (null != dt)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        Document.AddBaseDoc(baseDocId.ToString(), Document.PlatezhkiField.DocFieldId);
                        PaymentDocuments.SelectedItems.AddRange(dt.AsEnumerable().Select(rw =>
                        {
                            var id = ((int) rw["КодДокумента"]).ToString();
                            return new Lib.Entities.Item {Id = id, Value = PaymentDocuments.GetObjectById(id)};
                        }));
                    }
        ...
        protected void Invoice_ValueDeleted(object sender, ProperyDeletedEventArgs e)
        {
            if (!e.DelValue.IsNullEmptyOrZero())
                Document.RemoveBaseDoc(e.DelValue.ToInt(), Document.SchetPredField.DocFieldId);
        }
        ...
        protected void PaymentDocuments_ValueChanged(object sender, ProperyChangedEventArgs e)
        {
            ...
            {
                var value = e.NewValue.ToInt();
                Document.AddBaseDoc(value.ToString(), Document.PlatezhkiField.DocFieldId);

                var dtpList = GetControlTypeFilter(Document.SchetPredField.DocFieldId).Aggregate(string.Empty,
                    (current, dtp) => current + (current == string.Empty ? dtp.DocTypeID : "," + dtp.DocTypeID));
                var sql = SQLQueries.SELECT_СвязиДокументовВытекающиеИзОснованийПоТипу(e.NewValue,
                    Document.SchetPredField.DocFieldId.ToString(), dtpList);
                var dt = DBManager.GetData(sql, Config.DS_document);
                if (null != dt)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        var baseDocId = (int) row["КодДокумента"];
                        Document.AddBaseDoc(baseDocId.ToString(), Document.SchetPredField.DocFieldId);
                        Invoice.SelectedItems.AddRange(dt.AsEnumerable().Select(rw =>
                        {
                            var id = ((int) rw["КодДокумента"]).ToString();
                            return new Lib.Entities.Item {Id = id, Value = Invoice.GetObjectById(id)};
                        }));
                    }
        ...
        {
            if (!e.DelValue.IsNullEmptyOrZero())
                Document.RemoveBaseDoc(e.DelValue.ToInt(), Document.PlatezhkiField.DocFieldId);
        }
        ...
        {
            JS.Write("vagon_RecordsAdd('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}');",
                HttpUtility.JavaScriptStringEncode(title), pageId, docId, resultId, DBSShipperStore.Value,
                DBSPayerStore.Value, Shipper.Value, Payer.Value);
        }
        ...
        private void SetAccordionHeader(string panelId)
        {
            try
            {
        ...
        private void CopyMrisPosition(string copyId)
        {
        ...
        private void DeleteMrisPosition(string mrisId)
        {
            var mris = new Mris(mrisId);
            mris.Delete(false);
            RefreshMrisCurrentDoc(false, true);
        }
        ...
        private void CopyFactUslPosition(string copyId)
        {
            ...
            factUsl.Save(false);
            RefreshFactUslCurrentDoc(true, true);
        }
        ...
        private void DeleteFactUslPosition(string factUslId)
        {
            var factUsl = new FactUsl(factUslId);
            factUsl.Delete(false);
            RefreshFactUslCurrentDoc(false, true);
        }

        /// <summary>
        ///     Признак корректируемого документа
        /// </summary>
        private void FlagCorrecting_Uncheck(bool corr, string oldValue = "")
        {
            if (corr)
            {
        ...
        private void DetailMrisPosition(string mrisId)
        {
            var mris = new Mris(mrisId);
            ...
            else
            {
                ...
                if (!mris.ShipperStore.Id.IsNullEmptyOrZero())
                    lnkSkladFrom.Value = GetLntSkladLink("Shipper", Resx.GetString("TTN_lblSetCost"),
                        mris.ShipperStore.Name, Resx.GetString("TTN_lblResourceConsumption"), mris.Resource.Name,
                        mris.Count.ToString(), mris.Unit.ЕдиницаРус, mrisId);

                if (!mris.PayerStore.Id.IsNullEmptyOrZero())
                    lnkSkladTo.Value = GetLntSkladLink("Payer", Resx.GetString("TTN_lblSetCost"), mris.PayerStore.Name,
                        Resx.GetString("TTN_lblResourceConsumption"), mris.Resource.Name, mris.Count.ToString(),
                        mris.Unit.ЕдиницаРус, mrisId);
        ...
        private string GetLntSkladLink(string type, string cost, string payerStore, string resourceConsumption,
            string resourceName, string count, string unit, string mrisId)
        {
            ...
            return
                string.Format(
                    "<a href='#' onclick=\"{0}\" style='FONT-SIZE: 8pt; FONT-FAMILY: Verdana; COLOR: blue;'>{1}</a>",
                    string.Format("distrib_RecordsAdd('{0}','{1}','{2}','{3}','{4}');",
                        type,
                        cost + " " + payerStore + " " + resourceConsumption + " " + resourceName + " " + count + " " +
                        unit,
                        IDPage,
                        Document.Id,
                        mrisId
        ...
        public bool CheckIdentity(string resultGuid, out string info)
        {
            ...
            var dt = DBManager.GetData(SQLQueries.DeliveryFieldsRequest, Config.DS_document, CommandType.Text,
                sqlParams);
            Document.FillGPersonsDictionary(dt, ref values);

            if (values.ContainsKey("кодго") && Document.GOPersonField.Value != null)
            {
                var colGO = Convert.Str2Collection(values["кодго"]);
                switch (colGO.Count)
                {
                    case 0:
                        break;
                    case 1:
                        if (Document.GOPersonField.Value.ToString().Length > 0 &&
                            !colGO.Contains(Document.GOPersonField.Value.ToString()))
            ...
            {
                var colGP = Convert.Str2Collection(values["кодгп"]);
                switch (colGP.Count)
                {
                    case 0:
                        break;
                    case 1:
                        if (Document.GPPersonField.Value.ToString().Length > 0 &&
                            !colGP.Contains(Document.GPPersonField.Value.ToString()))
            ...
            {
                var colGOWesel = Convert.Str2Collection(values["узелотправления"]);
                switch (colGOWesel.Count)
                {
                    ...
                    case 1:
                        if (Document.GOPersonWeselField.Value.ToString().Length > 0 &&
                            !colGOWesel.Contains(Document.GOPersonWeselField.Value.ToString()))
                    ...
                    case 2:
                        if (Document.GOPersonWeselField.Value.ToString().Length > 0)
                        {
                            if (!colGOWesel.Contains(""))
                                distinctValue += (distinctValue.Length > 0 ? ", " : "") +
                                                 Document.GOPersonWeselField.Name;
                            sameValueTTN += (sameValueTTN.Length > 0 ? ", " : "") + Document.GOPersonWeselField.Name;
                        }
            ...
            {
                var colGPWesel = Convert.Str2Collection(values["узелназначения"]);
                switch (colGPWesel.Count)
                {
                    ...
                    case 1:
                        if (Document.GPPersonWeselField.Value.ToString().Length > 0 &&
                            !colGPWesel.Contains(Document.GPPersonWeselField.Value.ToString()))
                    ...
                    case 2:
                        if (Document.GPPersonWeselField.Value.ToString().Length > 0)
                        {
                            if (!colGPWesel.Contains(""))
                                distinctValue += (distinctValue.Length > 0 ? ", " : "") +
                                                 Document.GPPersonWeselField.Name;
                            sameValueTTN += (sameValueTTN.Length > 0 ? ", " : "") + Document.GPPersonWeselField.Name;
                        }
        ...
        protected override bool ValidateDocument(out List<string> errors, params string[] exeptions)
        {
            ...
            if ((exeptions.Contains("SaveButton") || exeptions.Contains("AddFactUsl") ||
                 exeptions.Contains("AddDocumentSign")) &&
                Document.PositionMris.Count == 0) errors.Add(Resx.GetString("TTN_msgNoMris"));
